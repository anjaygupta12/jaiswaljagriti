...
@section('title', 'Add Normal Advertisement')
@section('header', 'Add Normal Advertisement')
...
<div class="row">
    <div class="col-md-8 mx-auto">
        <div class="card card-box shadow-sm border-0">
            ...
            <div class="card-body">
                <form action="{{ route('admin.advertisement-normals.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                        ...
                        <div class="d-flex">
                            <select class="form-control @error('advertisement_type_id') is-invalid @enderror mr-2" id="advertisement_type_id" name="advertisement_type_id" required>
                                ...
                                @foreach($types as $type)
                                    <option value="{{ $type->id }}" {{ old('advertisement_type_id') == $type->id ? 'selected' : '' }}>{{ $type->name }}</option>
                                @endforeach
                            </select>
